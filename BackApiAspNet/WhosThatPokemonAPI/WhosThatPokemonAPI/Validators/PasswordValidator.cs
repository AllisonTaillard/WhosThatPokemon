        ...
        public override bool IsValid(object value)
        {
            ...
            else
            {
                ...
                if (!hasUpperLetters.IsMatch(input))
                    ErrorMessage += "Password should contain at least one upper case letters. ";
                if (!hasNumber.IsMatch(input))
                    ErrorMessage += "Password should contain at least two numeric characters. ";
                if (!hasSymbols.IsMatch(input))
                    ErrorMessage += "Password should contain at least one special characters. ";
